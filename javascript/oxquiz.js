...
function _quizInit(){
	parent.isConNext = true;	//말풍선 안뜨도록 초기화
...
$(document).ready(function(e) {		
	$("#review_btn1").bind("click",function(){
		...
		if (isNaN(parent.isMediaObj.duration))
		{
			_doNext2();
		} else {
...
function _doNext2(){
	thisParent._doLoadKnowHowAct();
}

function _doNext3(){
	thisParent._doLoadKnowHowAct2();
}

function _doNext4(){
	thisParent._doLoadKnowHowAct3();
}

function _doNext4(){
	thisParent._doLoadKnowHowAct4();
}

function _doNext4(){
	thisParent._doLoadKnowHowAct5();
}
...
function _endAct(){
	...
	if(isExp && isIEVersion<9){
		parent._doAlertSnd("../common/mp3/quiz/q_result.mp3");
	}else{
		effectSnd.volume = parent.isVolDragPercent;
		//alert("parent.isVolDragPercent = "+parent.isVolDragPercent);
	...
	if(myScore == 3){
		$("#endresult").html("기본기가 뛰어나네요. <br> 이번 시간 학습을 통해 좀 더 깊이 이해해 보세요.  ");
			try{
				
			}catch(e){
		}
	}else if( myScore < 3){		
		$("#endresult").html("아쉽네요. <br> 어떤 부분이 부족한지 확인하고 더 집중해서 학습해 보세요.");
			try{
				
			}catch(e){
		}
	}
	$("#scoreLine").html("총 3문제 중 "+myScore+"문제를 맞히셨습니다.");
	try{
...
function _popAlert(str){
	if(isExp && isIEVersion<9){
		...
		effectSnd.volume = Number(parent.isVolDragPercent);
		effectSnd.load(function(){
			effectSnd.play();
